...
function CheckBox({ label, addClass, name, formKey, id, value, dataset, inputValue, setInputValue }) {
    return (
        <div className={`form-check ${addClass}`}>
            <input className="form-check-input" name={name} value={value} data-set={dataset} checked={inputValue(formKey, id)} onChange={(e) => setInputValue(formKey, id, e.target.checked)} hidden type="checkbox" id={id} />
...
export default connect(mapStateToProps, { setInputValue })(CheckBox);
